...
constexpr Eigen::Matrix3d validRotationMatrix1T{
    {0, 1, 0},
    {-1,  0, 0},
    {0,  0, 1},
};

constexpr Eigen::Matrix3d invalidRotationMatrix1{
    {0, -1, -30},
    {1,  0, 1},
    {0,  4, 1},
};

TEST(GeometryTestSO3, ArithmeticWorks) {
    EXPECT_EQ(validRotationMatrix1T, validRotationMatrix1.transpose());
}

TEST(GeometryTestSO3, ConstructionPass) {
...
TEST(GeometryTestSO3, ConstructionFail) {
    EXPECT_ANY_THROW(geometry::RotationSO3{invalidRotationMatrix1});
}

TEST(GeometryTestSO3, Inverse) {
...
TEST(GeometryTestSO3, DefaultIsIdentity) {
    geometry::RotationSO3 r;
    EXPECT_EQ(r.matrix(), Eigen::Matrix3d::Identity());
}

TEST(GeometryTestSO3, MatrixAccessor) {
    geometry::RotationSO3 r(validRotationMatrix1);
    EXPECT_EQ(r.matrix(), validRotationMatrix1);
}

TEST(GeometryTestSO3, ActsOnVector) {
...
TEST(GeometryTestSO3, Composition) {
...
TEST(GeometryTestSO3, LogAtPiAxisAligned) {
...
TEST(GeometryTestSO3, LogAtPiOffAxis) {
...
TEST(GeometryTestSO3, RejectsReflection) {
...
TEST(GeometryTestSO3, ExpTinyAngleTaylorBranch) {
...
TEST(GeometryTestSO3, ExpAtBoundaryAngle) {
...
TEST(GeometryTestSO3, HatVeeRoundTrip) {
    const Eigen::Vector3d w(1, 2, 3);
    const Eigen::Matrix3d W = geometry::hat(w);
